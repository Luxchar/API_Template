...
const userBan = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (!utils_1.default.FUNCTIONS.CHECK.SERVER.PERMISSIONS(User, Server, utils_1.default.CONSTANTS.SERVER.PERMISSIONS.ADMIN))
            throw "You do not have permission to ban users";
        // remove the user from the server members array of maps
        delete Server.members[parseInt(user_id)];
        // add the user to the server banned members
        if (!Server.banned_users)
            Server.banned_users = []; // if banned_users is undefined, set it to an empty array
        Server.banned_users.push(parseInt(user_id));
        emitter_client_1.default.emit("updateBan", Server);
